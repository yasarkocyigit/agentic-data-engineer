...
Submits Spark jobs to the remote Spark cluster using spark-submit via BashOperator.
...
# Note: AWS credentials and endpoint are passed here for backup, 
# but the script's internal SparkSession builder is the primary source of truth
# to handle the specific Docker network constraints (SimpleAWSCredentialsProvider).
SPARK_CONF_CMD = (
...
with DAG(
    ...
    # We invoke spark-submit directly from the Airflow container.
    # The Docker image now includes pyyaml and psycopg2-binary, so no pip install needed here.
